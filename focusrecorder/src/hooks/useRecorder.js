 ...
 *
 * ROOT CAUSE OF 1–2 SECOND BUG:
 * Chrome's MediaRecorder writes a small initial duration (e.g. 1000–2000ms) into the
 * EBML Info Header (Duration element 0x4489) when recording finishes or flushes chunks.
 * Standard libraries like fix-webm-duration check `if (existingDuration <= 0)` and skip
 * updating if a non-zero duration exists. As a result, 10-second recordings retain Chrome's
 * 1–2 second header metadata, causing HTML5 video players to stop playing after 1–2s.
 *
 * FIX: Parse the WebM EBML tree (Segment -> Info -> TimecodeScale & Duration) and directly
 * set the Duration float to (durationMs * 1,000,000 / timecodeScale).
 */
function setWebmDuration(arrayBuffer, durationMs) {
  ...
  while (offset < bytes.length - 4) {
    ...
    if (idInfo.id === 0x18538067) {
      segmentEnd = offset + sizeInfo.value;
      while (offset < segmentEnd && offset < bytes.length - 4) {
        ...
        if (segIdInfo.id === 0x1549A966) {
          const infoEnd = offset + segSizeInfo.value;
    ...
    try {
      ...
      if (hasSysAudio && hasMicAudio) {
        ...
        try {
          ...
          // Keep refs to prevent V8 GC from disconnecting nodes
          audioCtxRef._sysSource = sysSource;
          audioCtxRef._micSource = micSource;
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          // FIX: Use the stop time captured at the exact moment stop was requested,
          // not Date.now() here which fires after async onstop delay.
          const stopTime = recordingStopTimeRef.current || Date.now();
          const wallClockMs = Math.max(0, stopTime - startTime);
          const durationMs = accumulatedMsRef.current > 0 ? accumulatedMsRef.current : Math.max(1000, wallClockMs);
